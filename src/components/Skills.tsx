  ...
  return (
    <section id="skills" className="py-24 relative overflow-hidden bg-white">
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
          ...
          {(() => {
            const logoItems: LogoItem[] = skillCategories.flatMap((category) =>
              category.skills.map((skill) => ({
                node: (
                  <div className="text-sm font-medium px-6 py-3 rounded-xl bg-white/5 border border-black/10 text-zinc-700 shadow-md flex items-center gap-3 hover:text-black hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 cursor-default backdrop-blur-sm">
          ...
          {skillCategories.map((category, index) => (
            ...
            >
              <div className="flex items-center gap-3 border-b border-black/5 pb-4">
                <div className="p-2.5 rounded-xl bg-white/5 border border-black/10">
                  {category.icon}
                </div>
                <h3 className="font-bold text-black tracking-wide text-lg">
                  {category.title}
                </h3>
              </div>
              ...
              <div className="flex flex-wrap gap-2.5">
                {category.skills.map((skill) => (
                  <span
                    key={skill}
                    className="text-xs font-medium px-3 py-1.5 rounded-lg bg-white/5 border border-black/5 text-zinc-700 hover:text-black hover:border-primary/40 hover:bg-primary/5 transition-all duration-300 cursor-default"
                  >
                    {skill}
                  </span>
                ))}
        ...
        {/* Extra details strip */}
        <div className="mt-12 p-6 glass rounded-2xl border border-black/5 flex items-center gap-6">
          <div className="flex items-center gap-4">
            <div className="p-3 rounded-xl bg-secondary/10 text-secondary hidden md:block">
              <Cpu className="w-6 h-6 animate-pulse" />
            </div>
            <div>
              <h4 className="font-semibold text-black text-base">Academic Excellence</h4>
              <p className="text-sm text-zinc-600">Bachelor of Science in Computer Engineering. Multi-year Dean's Lister (2022, 2023).</p>
            </div>
          </div>
        </div>
        </AnimatedContent>
