...
const GooeyNav: React.FC<GooeyNavProps> = ({
  ...
  const updateEffectPosition = (element: HTMLElement) => {
    ...
    // Copy the SVG icon HTML instead of plain text label
    textRef.current.innerHTML = element.innerHTML;
  };

  const handleClick = (e: React.MouseEvent<HTMLAnchorElement | HTMLButtonElement>, index: number, onClickCallback?: () => void) => {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLAnchorElement | HTMLButtonElement>, index: number, onClickCallback?: () => void) => {
        ...
        <nav className="flex relative" style={{ transform: 'translate3d(0,0,0.01px)' }}>
          ...
          >
            {items.map((item, index) => (
              <li
                key={index}
                className={`rounded-full relative cursor-pointer transition-[background-color_color_box-shadow] duration-300 ease flex items-center justify-center ${
